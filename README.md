...
**Método:** `POST`
- **Campos:**
  - `login` (obrigatório): Necessário para informar que você quer realizar login.
  - `email` e `senha` (obrigatório se `token` não for fornecido): Endereço de email e senha do usuário.
  - `token` (obrigatório se `email` e `senha` não forem fornecidos): Token para autenticação automática.
...
**Método:** `POST`
- **Campos:**
  - `logout` (obrigatório): Necessário para informar que você quer realizar logout.
```bash
curl -X POST "http://localhost/restaurante/back/api/usuario.php" \
-H "Content-Type: application/json" \
-H "Authorization: Bearer abcdef123456" \
...
```json
{
  ...
  "dados": { ... } // dados será sempre um array associativo
}
```
---
## Requisitos
